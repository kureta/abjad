...
#        specialized composition-specific logic. `SelectionAgent.append_callback()`
...
#
#                >>> from abjad.tools import abctools
#                >>> class CMajorSelectionAgentCallback(abctools.AbjadValueObject):
...
#                >>> selector = abjad.select()
#                >>> selector = selector.by_leaf()
#                >>> selector = selector.append_callback(CMajorSelectionAgentCallback())
#
#            ::
#
#                >>> selector(staff)
...
#        return selector
    ...
    def get_item(self, n):
        ...
        if self._expression:
            return self._update_expression(inspect.currentframe())
        return self.client.__getitem__(n)
    ...
    def get_slice(self, start=None, stop=None):
        ...
        ..  container:: example
            ...
            ::
                ...
                >>> selector = abjad.select()
                >>> selector = selector.by_logical_tie(pitched=True)
                >>> selector = selector.map(abjad.select().by_leaf()[1:])
        ...
        return self.client.__getitem__(slice_)
